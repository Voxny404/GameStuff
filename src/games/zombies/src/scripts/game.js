...
const createZombies = () => {
  let zombie1 = {
    ...
    width:60, height:60, id: 'zombie1_' + zombieCounter, overlay:1
  }
...
let player = {
  ...
  width:60, height:60, id: 'player', overlay:2, angle: 0, mouseX: 0, mouseY:0
};

imageLoader.loadImage('./src/images/map.png');
imageLoader.createScoreBoard();
imageLoader.createImageObject(player.id);
...
const gameLoop = () => {
  ...
  zombies.forEach((item, i) => {
    ...
    imageLoader.updateImageObject(item);
  });

  imageLoader.updateImageObject(player);
};
...
const createShoot = (e) => {
  if(!gameOver) {
      ...
      setTimeout(() => imageLoader.destroy('shoot'), 50);
  }
